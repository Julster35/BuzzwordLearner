...
This preprocessing step addresses real-world data quality issues (inconsistent capitalization, formatting artifacts) and improves matching accuracy by approximately 15-20\% compared to raw text matching.
...
\midrule
Department Accuracy & -- & 0.771 \\
Department F1 (macro) & -- & 0.572 \\
Department F1 (weighted) & -- & 0.755 \\
\midrule
Seniority Accuracy & -- & 0.542 \\
Seniority F1 (macro) & -- & 0.474 \\
Seniority F1 (weighted) & -- & 0.584 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Department classification} achieves 77.1\% accuracy, substantially exceeding the naive baseline of 55\% (always predicting "Other")
    \item \textbf{Seniority classification} reaches 54.2\% accuracy, outperforming the naive baseline of 35\% (always predicting "Professional")
    \item The weighted F1 scores are notably higher than macro F1, indicating better performance on frequent classes
    \item Performance gap between in-distribution and real-world data reveals the challenge of generalizing from lookup tables to natural job title variations
\end{itemize}
