...
\begin{itemize}
    \item A Macro F1 of 0.615 (Feature Engineering) implies that the model successfully distinguishes between specific departments (like Sales, IT, HR) with balanced precision and recall, rather than just maximizing the majority class.
    \item This metric confirms that the system is practically useful for routing candidates to specific departmental pools, whereas a high-accuracy model (like Two-Stage DistilBERT, 68.5\% acc, 0.535 F1) might achieve its score by conservatively over-predicting "Other".
\end{itemize}
