...
The analysis of department stability shows the following distribution:
\begin{itemize}
...
Analysis of the 15 most common department transitions reveals that they either lead from or to the "Other" category. Therefore, no clear patterns in department changes can be identified, suggesting heterogeneous career development across different functional areas.
...
\begin{figure}[H]
    ...
    \includegraphics[width=1\textwidth]{figures/title_length_distribution.png}
    \label{fig:title_length_distribution}
\end{figure}
...
A simple heuristic method was applied to detect the language of position titles. The most commonly native languages in Europe are Russian, German, French, English, and Turkish. Since the dataset contains no Cyrillic characters, Russian was excluded. The remaining languages were broadly categorized.
...
After filtering to only active positions (current jobs), the following picture emerges:
...
The vast majority (79.48\%) of candidates have only one active job, while 16.32\% hold two concurrent positions. This could indicate side jobs, consulting activities, or part-time work.
...
\textbf{Modeling implication}: The dominance of "Other" as a catch-all category requires class weighting or resampling techniques to avoid bias toward the majority class.
...
The absence of the \textit{Professional} level in the lookup table, despite its prevalence (34.7\%) in the annotated dataset, presents a challenge for model development.
...
These similarities strongly suggest that models trained on the annotated dataset will generalize well to the unannotated data for inference. The primary difference lies in the absence of ground truth department and seniority labels, which are the targets for prediction. This consistency validates the representativeness of the annotated training data and supports confidence in deployment to the larger unannotated dataset.
...
\textbf{Key Observations:}
\begin{itemize}
    \item \textbf{Moderate Department Improvement}: The department accuracy of 44.4\% and F1 of 0.438 improves over the baseline DistilBERT (F1=0.343), suggesting that exposing the model to real CV patterns helps it handle the domain shift better than training on lookup tables alone.
    \item \textbf{Lower Seniority Performance}: For seniority, the approach achieved 48.3\% accuracy, which is significantly lower than the baseline DistilBERT (70.5\%). This indicates that the strict confidence filtering might have excluded valuable difficult examples, or that the model's confidence on seniority was not well-calibrated on the target domain.
\end{itemize}
\subsection{Conclusion}
\label{subsec:pseudo-conclusion}
Pseudo-labeling yielded mixed results. While it provided a clear benefit for department classification by bridging the domain gap, it degraded performance for seniority. This divergent behavior highlights a key risk of semi-supervised learning: if the initial model's calibration is poor on out-of-distribution data (as seen with the seniority domain gap), the high-confidence filter may select incorrect labels or bias the distribution, leading to worse final performance. However, for tasks where the initial model has a reasonable grasp of the target domain (like departments), pseudo-labeling offers a viable path to improved generalization without manual annotation.
...
\begin{itemize}
    ...
    \item \textbf{Domain gap}: Despite 94\% in-distribution validation performance, real-world test performance drops to 24-44\%, revealing significant generalization challenges
    \item \textbf{Department classification}: Best department F1 is 0.354 (Two-Stage DistilBERT), outperforming Feature Engineering's corrected result of 0.248
\end{itemize}
...
Feature Engineering & 0.282 & 0.248 & 0.475 & 0.443 \\
\midrule
...
\textbf{Key Findings:}
\begin{enumerate}
    \item \textbf{Department Classification}: Two-Stage DistilBERT achieves the best F1 (0.535) and the best accuracy (68.5\%) overall in the standardized real-world evaluation.
...
The Two-Stage DistilBERT approach (F1=0.535) outperformed Feature Engineering (F1=0.248) once the "Other" class was correctly accounted for. The success of the hierarchical strategy is attributed to:
...
For department classification, \textbf{Accuracy is a misleading metric}. Due to the 55\% prevalence of the "Other" category, a trivial "dummy" classifier predicting "Other" for every input would achieve 55\% accuracy but offer zero business value.
\begin{itemize}
    \item A Macro F1 of 0.354 (Two-Stage DistilBERT) implies that while the model faces challenges with the dominant "Other" class, it maintains the highest discriminative power among all tested variants.
...
\end{itemize}
\paragraph{Seniority Reliability}
For seniority, the alignment between Accuracy (70.5\%) and Macro F1 (0.616) provides strong evidence of reliability. Since the class distribution is less skewed, high performance on both metrics confirms that the model is not biased toward a single level (e.g., just predicting "Professional"). A predictive accuracy of 70\% on 5 classes is approximately 3.5x better than random chance, indicating a strong signal for automated candidate screening.
...
\paragraph{Department Prediction: Feature Engineering (Random Forest)}
We clearly recommend the **Feature Engineering** approach (Section \ref{sec:feature-engineering}) for department classification.
\begin{itemize}
    \item \textbf{Justification}: This model achieved the highest Macro F1 score (0.615), outperforming all transformer-based approaches (best DistilBERT F1: 0.535). Crucially, it demonstrated the strongest robustness to domain shift, with only a 24\% performance drop from training to real-world data (compared to $\approx$70\% for text-based models).
    \item \textbf{Operational}: As a Random Forest model, it offers negligible inference latency ($<$10ms) and does not require GPU acceleration, making it highly cost-effective for high-throughput processing.
    \item \textbf{Integration}: We recommend using the Rule-Based classifier (Section \ref{sec:rule-based-baseline}) as a pre-filter. If the Rule-Based system finds an exact match in the lookup table, that label is trusted (accuracy near 100\% for known titles). If no match is found, the request is routed to the Feature Engineering model.
\end{itemize}
\paragraph{Seniority Prediction: DistilBERT Baseline}
We recommend the **DistilBERT Baseline** (Section \ref{sec:distilbert-models}) for seniority classification.
\begin{itemize}
    \item \textbf{Justification}: This model dominates all other approaches with an accuracy of 70.5\% and Macro F1 of 0.616. Unlike department classification, seniority relies on explicit lexical markers ("Senior", "Lead", "Director") that the transformer attention mechanism captures effectively without complex manual feature extraction.
    \item \textbf{Operational}: While computationally more expensive than feature engineering, the superior performance (+17 percentage points in F1 over Feature Engineering) justifies the resource cost. For latency-sensitive applications, model distillation or quantization could be applied.
    \item \textbf{Fallbacks}: The "Professional" fallback used in the Rule-Based approach should \textbf{not} be used in the final system, as it relies on data leakage (see Section \ref{subsec:fallback-dilemma}). DistilBERT provides a valid probability distribution across all classes, eliminating the need for arbitrary defaults.
\end{itemize}
